module ParseErrors where
  ...
  main = do
    ...
    Trace.trace $ case parseJSON "not even JSON" of
    ...
    Trace.trace $ case parseJSON "26" of
      Left err -> "Error parsing JSON:\n" ++ err
    ...
    Trace.trace $ case parseJSON "[1, true, 3]" of
      Left err -> "Error parsing JSON:\n" ++ err
      Right result -> show $ result :: [Boolean]
    ...
    -- failed to parse was, along with the reason the value didn't parse.
    Trace.trace $ case parseJSON "{ \"x\": 1, \"y\": false }" of
      Left err -> "Error parsing JSON:\n" ++ err
