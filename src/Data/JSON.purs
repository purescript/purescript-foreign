...
foreign import showJSONImpl "var showJSONImpl = JSON.stringify;" :: JSON -> String
...
instance readJSONArray :: (ReadJSON a) => ReadJSON [a] where
  readJSON =
    let arrayItem (Tuple i x) = case runParser readJSON x of
      Right result -> Right result
      Left err -> Left $ "Error reading item at index " ++ (show i) ++ ":\n" ++ err
    in
    (JSONParser $ readPrimType "Array") >>= \xs -> JSONParser \_ ->
      arrayItem `mapM` (zip (range 0 (length xs)) xs)
